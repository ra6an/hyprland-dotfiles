...
## Get data
get_weather_data() {
	weather=`curl -sf "https://api.openweathermap.org/data/2.5/weather?APPID="$KEY"&id="$ID"&units="$UNIT""`
	echo ${weather}
	...
	if [ ! -z "$weather" ]; then
		...
		# printf "%s" "$weather_icon" > ${cache_weather_icon}
		# echo -n "$weather_icon" >  ${cache_weather_icon}
		echo "$weather_icon" >  ${cache_weather_icon}
		# echo "$cache_weather_icon_code"
